...
|---|---|---|---|---|---|---|---|---|---|
| request.created | business | yes | bottom-right | Отдельный producer в `RequestService.create_request` не найден | Наблюдатели/владелец заявки (требует уточнения) | `/requests/{request_id}` | `request_id`, `actor_user_id` | todo | Заявка создается, но бизнес-событие уведомления не публикуется. |
...
| request.responsible_changed | business | yes | bottom-right | Смена владельца есть в `update_request`, но событие уведомления не публикуется | Старый/новый ответственный (требует уточнения) | `/requests/{request_id}` | `old_owner_user_id`, `new_owner_user_id`, `actor_user_id` | partial | Изменение данных есть, контракта уведомления нет. |
| request.deadline_changed | business | yes | bottom-right | Обновление дедлайна есть в `update_request`, но событие не публикуется | Участники заявки (требует уточнения) | `/requests/{request_id}` | `old_deadline`, `new_deadline`, `actor_user_id` | partial | Обновление есть, отдельного уведомления нет. |
...
| offer.accepted | business | yes | bottom-right | `OfferService.update_status` поддерживает accepted, но уведомление не публикуется | Владелец КП / владелец заявки (требует уточнения) | `/offers/{offer_id}/workspace` | `offer_id`, `request_id`, `actor_user_id` | todo | Изменение статуса есть, события уведомления нет. |
| offer.rejected | business | yes | bottom-right | `OfferService.update_status` поддерживает rejected, но уведомление не публикуется | Владелец КП / владелец заявки (требует уточнения) | `/offers/{offer_id}/workspace` | `offer_id`, `request_id`, `actor_user_id` | todo | Изменение статуса есть, события уведомления нет. |
| offer.deleted | business | yes | bottom-right | `OfferService.update_status` поддерживает deleted, но уведомление не публикуется | Владелец КП / владелец заявки (требует уточнения) | `/offers/{offer_id}/workspace` | `offer_id`, `request_id`, `actor_user_id` | todo | Событие удаления как уведомление не реализовано. |
