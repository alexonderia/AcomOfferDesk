...
class UnitService:
    ...
    async def _resolve_root_unit_id(self, *, unit: Unit) -> int:
        ...
        while cursor.id_parent is not None:
            ...
            if parent is None:
                raise Conflict("У подразделения отсутствует родительский узел")
            cursor = parent
    ...
    async def _ensure_unique_name(
        ...
        *,
        ...
        if exclude_unit_id is not None and int(sibling.id) == exclude_unit_id:
            return
        raise Conflict("Подразделение с таким названием уже существует на этом уровне")
    ...
    async def _build_single_unit_state(
        ...
        *,
        ...
        depth = await self._get_unit_depth(unit=unit)
        has_active_children = bool(await self._units.list_children(unit_id=int(unit.id), active_only=True))
        return UnitNodeState(
            ...
            children=[],
            actions=UnitActionsState(
                can_create_child=can_manage and bool(unit.is_active) and depth < self.MAX_DEPTH,
                can_update=can_manage and bool(unit.is_active),
                can_deactivate=can_manage and bool(unit.is_active) and not has_active_children,
    ...
    async def get_tree(self, *, current_user: CurrentUser) -> list[UnitNodeState]:
        ...
        async def _build_node(unit: Unit) -> UnitNodeState:
            ...
            return UnitNodeState(
                ...
                children=children,
                actions=UnitActionsState(
                    can_create_child=can_manage and depth < self.MAX_DEPTH,
                    can_update=can_manage,
                    can_deactivate=can_manage and not children,
    ...
    async def create_unit(
        ...
        *,
        ...
        if not normalized_name:
            raise Conflict("Название подразделения обязательно")
        ...
        if id_parent is None:
            ...
            if parent_unit is None:
                raise NotFound("Родительское подразделение не найдено")
            if not parent_unit.is_active:
                raise Conflict("Нельзя создавать узел внутри неактивного подразделения")
            await self._ensure_manage_access(current_user=current_user, unit=parent_unit)
            if (await self._get_unit_depth(unit=parent_unit)) >= self.MAX_DEPTH:
                raise Conflict("Нельзя создавать подразделение глубже 3 уровней")
    ...
    async def update_unit(
        ...
        *,
        ...
        if name is None and is_active is None:
            raise Conflict("Нет данных для обновления подразделения")
        ...
        if is_active is True and not unit.is_active:
            raise Conflict("Повторная активация подразделения пока не поддерживается")

        if is_active is False and unit.is_active:
            active_children = await self._units.list_children(unit_id=int(unit.id), active_only=True)
            if active_children:
                raise Conflict("Сначала деактивируйте дочерние подразделения")
            unit.is_active = False
            member_rows = await self._units.list_members(unit_id=int(unit.id), active_only=True)
            for member, _user, _profile, _role in member_rows:
    ...
    async def add_member(
        ...
        *,
        ...
        if unit is None:
            raise NotFound("Подразделение не найдено")
        if not unit.is_active:
            raise Conflict("Нельзя добавлять участников в неактивное подразделение")
        await self._ensure_manage_access(current_user=current_user, unit=unit)
        ...
        membership = await self._units.get_member(unit_id=unit_id, user_id=user_id)
        if membership is not None and membership.is_active:
            raise Conflict("Пользователь уже добавлен в это подразделение")
        ...
        raise Conflict("Не удалось добавить пользователя в подразделение")
    ...
    async def remove_member(
        ...
        *,
        ...
        if membership is None or not membership.is_active:
            raise NotFound("Участник подразделения не найден")
    ...
    async def list_available_users_for_unit(
        ...
        *,
        ...
        if unit is None:
            raise NotFound("Подразделение не найдено")
        if not unit.is_active:
            raise Conflict("Для неактивного подразделения нельзя подбирать участников")
        await self._ensure_manage_access(current_user=current_user, unit=unit)
