...
def _iter_chunks(content_bytes: bytes, chunk_size: int) -> list[bytes]:
    if chunk_size <= 0:
        raise ValueError("chunk_size must be positive")
    return [content_bytes[index : index + chunk_size] for index in range(0, len(content_bytes), chunk_size)]
