...
class RegistrationInviteTokenCodec:
    def __init__(self, secret: str | None = None, ttl_seconds: int | None = None) -> None:
        ...
        self._codec = EmailVerificationTokenCodec(secret=secret or settings.email_verification_secret, ttl_seconds=self._ttl_seconds)

    def issue(self, *, email: str, role_id: int, inviter_id: str, unit_id: int | None = None) -> str:
        return self._codec.encode_payload({"purpose": INVITATION_PURPOSE, "email": email, "role_id": role_id, "unit_id": unit_id, "inviter_id": inviter_id, "nonce": secrets.token_urlsafe(16), "exp": int((datetime.now(timezone.utc) + timedelta(seconds=self._ttl_seconds)).timestamp())})

    def parse(self, token: str) -> RegistrationInviteClaims:
        try:
            payload = self._codec.decode_payload(token)
        except Unauthorized as exc:
            if "expired" in str(exc).lower() or "истёк" in str(exc):
        ...
        try:
            role_id, exp = int(payload.get("role_id")), int(payload.get("exp", 0))
        ...
        return RegistrationInviteClaims(purpose=purpose, email=email, role_id=role_id, unit_id=unit_id, inviter_id=inviter_id, nonce=nonce, exp=exp)
